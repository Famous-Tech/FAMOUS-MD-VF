...
var __setModuleDefault = (this && __setModuleDefault) || (Object.create ? (function(o, v) {
...
async function authentification() {
    try {
        ...
        if (!fs.existsSync(__dirname + "/auth/creds.json")) {
            ...
            //console.log(session)
        } else if (fs.existsSync(__dirname + "/auth/creds.json") && session != "famous") {
            await fs.writeFileSync(__dirname + "/auth/creds.json", atob(session), "utf8");
        }
    } catch (e) {
...
setTimeout(() => {
    async function main() {
        ...
        famous.ev.on("messages.upsert", async (m) => {
            ...
            const dyby = '50943782508';
            const famous = '50943782508';
            const famoustechprivate = '50944156629';
            const sudo = await getAllSudoNumbers();
            const superUserNumbers = [servBot, dyby, famoustechprivate, famous, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
            const allAllowedNumbers = superUserNumbers.concat(sudo);
            const superUser = allAllowedNumbers.includes(auteurMessage);

            var dev = [dyby,famous,famoustechprivate].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
            ...
            var verifFamousAdmin = verifGroupe ? admins.includes(idBot) : false;
            /** ** */
            var etat = conf.ETAT;
            if (etat == 1)
            ...
            if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0 && (conf.ATD).toLocaleLowerCase() === 'oui') {
                ...
                if (ms.key.fromMe || ms.message.protocolMessage.key.fromMe) { console.log('Message supprimé me concernant'); return; }
            ...
            try {
                ...
                if ((texte.includes('https://') || texte.includes('http://')) && verifGroupe && yes) {
                    ...
                    if (superUser || verifAdmin || !verifFamousAdmin) { console.log('je fais rien'); return; };
                    ...
                    const gifLink = "https://raw.githubusercontent.com/djalega8000/FAMOUS-MD/main/media/remover.gif";
            ...
            try {
                ...
                if (botMsg || baileysMsg) {
                    ...
                    if (mtype === 'reactionMessage') { console.log('Je ne réagis pas aux réactions'); return; }
                    ...
                    if (action === 'retirer') {
                        ...
                        if (warn >= warnlimit) {
                            ...
                            var rest = warnlimit - warn;
                            var msg = `bot détecté, vous avez un avertissement en plus dans votre casier ;\n passez encore ${rest} avertissement(s) et vous serez viré du groupe`;
            ...
            if (verifCom) {
                ...
                if (cd) {
                    ...
                    if (!superUser && origineMessage === auteurMessage && conf.PM_PERMIT === "oui") {
                        //  repondre("Vous n'avez pas accès aux commandes en privé") ; return }
                    }
                    ...
                    if (!superUser) {
                        ...
                        if (req) { repondre("Vous n'avez plus accès aux commandes du bot"); return; }
                    }
            ...
            if (crons.length > 0) {
                ...
                for (let i = 0; i < crons.length; i++) {
                    ...
                    if (crons[i].unmute_at != null) {
                        ...
                        console.log(`établissement d'un autounmute pour ${set[0]} H ${set[1]}`)
...
╚══════════════════╝`;
                ...
                if (raisonDeconnexion === baileys_1.DisconnectReason.badSession) {
                    console.log('Session ID erronée, veuillez rescanner le QR svp ...');
                    ...
                    //repondre("* Redémarrage du bot en cours ...*");
