...
setTimeout(() => {
    async function main() {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const dev = [dj, dj2, dj3, dj4, luffy].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
            ...
            if (conf.CHAT_BOT === 'oui') {
                ...
                fetch(`http://api.brainshop.ai/get?bid=177607&key=NwzhALqeO1kubFVD&uid=[uid]&msg=${arg.join(' ')}`)
                    ...
                    .then(data => {
                        ...
                        if (superUser) console.log(botResponse);
            ...
            if (ms && ms.message.stickerMessage) {
                ...
                if (!inStickCmd(id) || !superUser) return;
            ...
            try {
                ...
                if (botMsg || baileysMsg) {
                    if (mtype === 'reactionMessage') { console.log('Je ne reagis pas aux réactions'); return; }
            ...
            if (verifCom) {
                ...
                if (cd) {
                    if (conf.MODE != 'oui' && !superUser) return;
                    if (!dev && origineMessage == "120363158701337904@g.us") return;
                    // PM_PERMT
                    if (!superUser && origineMessage === auteurMessage && conf.PM_PERMIT === "oui") return;
                    // banGroup
        ...
        zk.ev.on('group-participants.update', async (group) => {
            ...
            try {
                ...
                if (group.action == 'add' && (await recupevents(group.id, "welcome") == 'oui')) {
                    let msg = `🎉 Bienvenue à toi, nouveau membre ! 🎉

👋 Salut @${group.participants[0].split("@")[0]}, nous sommes ravis de t'accueillir parmi nous !

🌟 N'hésite pas à explorer le groupe et à partager tes idées. Nous sommes une communauté dynamique et accueillante.

🚀 Profite bien de ton séjour parmi nous !

👉 *Description du groupe :*
${metadata.desc}`;
                    zk.sendMessage(group.id, { image: { url: ppgroup }, caption: msg, mentions: group.participants });
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                var md;
                if ((conf.MODE).toLowerCase() === "oui") md = "public";
                else if ((conf.MODE).toLowerCase() === "non") md = "privé";
                else md = "indéfini";
                console.log("chargement des commandes terminé ✅");
                await activateCrons();
                if ((conf.DP).toLowerCase() === 'oui') {
                    const master = conf.NOM_OWNER;
                    const masternum = conf.NUMERO_OWNER;
                    let cmsg = `╔════◇
║ 『FAMOUS-𝐌𝐃』
║    Prefix : [ ${prefixe} ]
║    Mode : ${md}
║    Nombre total de Commandes : ${evt.cm.length}
║    Nom Propriétaire : ${master}
║    Numéro Propriétaire : ${masternum}︎︎︎
╚══════════════════╝
