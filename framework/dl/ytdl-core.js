...
class YT {
    ...
    static downloadMusic = async (query) => {
        try {
            ...
            const videoInfo = await ytdl.getInfo('https://www.youtube.com/watch?v=' + search.id, { lang: 'id', agent: yt_Agent(ytdl) });
            let stream = ytdl(search.id, { filter: 'audioonly', quality: 140 , agent: yt_Agent(ytdl)});
    ...
    static mp4 = async (query, quality = 134) => {
        try {
            ...
            const videoInfo = await ytdl.getInfo('https://www.youtube.com/watch?v=' + videoId, { lang: 'id' , agent: yt_Agent(ytdl) });
            const format = ytdl.chooseFormat(videoInfo.formats, { format: quality, filter: 'videoandaudio', agent: yt_Agent(ytdl) })            
            return {
    ...
    static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
        try {
            ...
            const { videoDetails } = await ytdl.getInfo(url, { lang: 'id' , agent: yt_Agent(ytdl) });            
            let stream = ytdl(url, { filter: 'audioonly', quality: 140 , agent: yt_Agent(ytdl)});
